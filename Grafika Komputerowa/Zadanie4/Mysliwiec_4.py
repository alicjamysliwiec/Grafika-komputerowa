...
def render(time):
    ...
    if key_rotate:
        ...
        if not right_mouse_button_pressed:
            scale = 1;
        glRotatef(phi/2*math.pi * 45, 1.0, 0.0, 0.0)
        glRotatef(theta/2*math.pi * 45, 0.0, 1.0, 0.0)
    ...
    if not key_rotate:
        ...
        viewer[0] = xeye(R, theta, phi)
        viewer[1] = yeye(R, theta, phi)
        viewer[2] = zeye(R, theta, phi)
        gluLookAt(viewer[0], viewer[1], viewer[2], 0.0, 0.0, 0.0, 0.0, R_start, 0.0)
